...
! Definitions for the observational data
  INTEGER, PARAMETER  :: maxobs=33000, nhor=120
  INTEGER :: nobs_tot
...
! Local work variables
  ...
  character(len=20) :: outfile1, outfile2
  integer :: date, nobs, i,j,k, nm_lats
...
! regular Gaussian grid of the input file.
  ...
  call get_command_argument(2,gribdir)
  call get_command_argument(3,resdir)
  print*,' ... done'
  ierr=0
...
!
    ...
    outfile2=trim(outfile1) // '+' // trim(hour) // '.dat'

    print*,'####### Writing the sky view for station ',stations(i),' #######'
    call writeDelays2(resdir, stations(i), lat, lon, hei, sdelay_nwp, outfile2, date, hour_vt, zen, azi)
    do j=1,nobs
      if (sdelay_nwp(i) /= sdelay_nwp(i)) print*,'!!!!!!! NaNs detected !!!!!!!'
    enddo
    print*,'  ... Done!'
  enddo
...
! 7. Memory is freed, and the file closed.
  ...
  subroutine getFields2(gribdir,gfname,nx,ny,nlev,afull,bfull,fis_bg,lnps_bg,t_bg,q_bg,ierr)
    CHARACTER(LEN=90) :: gribdir, gfname
    character(len=110) filename
    INTEGER :: nx, ny, nlev, paramId
    REAL :: afull(nlev), bfull(nlev)
    ...
    character(len=90) :: coordsfile='LTT_conf_files/vert_coord_l91.dat'
...
! 6.1.
      if (.not. switch .and. paramId==130) then
        switch=.true.
        jz=1
      elseif (.not. switch .and. paramId==133) then
        switch=.true.
        jz=1
      end if
! 6.2.
...
! These vectors are later used element by element in the slant delay calculations.
  ...
  subroutine createSlantGPSfield(station, longitude, latitude, height, nrays, stid, lats, lons, heis, izen, iazi, aa_geom, za_geom)
    implicit none
...
! The azimuth circles begin from 1 deg and run to 360 deg.
  subroutine writeDelays2(resdir, statid, zlat, zlon, zhei, zstd, outfile, idate, itime, izen, iazi)
    implicit none
    ...
    istart=istop-2*iazi
    do i=istart,istop
      if (zstd(i)<0.0 .or. zstd(i)>70.0 .or. zstd(i)/=zstd(i)) then
        zstd(i)=0.0
    ...
    write(12,'(F23.18)') zstd(1)
    do iz = 1, izen
      do ia = 1, iazi   
        zstd_a(ia)=zstd(counter)
        counter=counter+1      
      enddo
      write(12,'(360(F23.18,3X))') zstd_a
    enddo
    write(12,'(1X,A4)') eofr
...
!
  SUBROUTINE slantGPS_ltt( &
    ...
    INTEGER :: nx, ny, nlev
    REAL :: dlon
    REAL, DIMENSION(nlev) :: afull, bfull
    ...
    REAL, DIMENSION(maxobs) :: sdelay_top, sdelay_mod
...
!
    ...
    real :: gravity1, gravity2, dz1, dz2
!
!
!
! Loop over the observations
...
!
! * Pressure temperature and specific humidity at the model levels
!
          DO jlev=1,nlev
             p_pr(jobs,jlev,jhor) = afull(jlev) + bfull(jlev)*EXP(lnps_ip)
             t_pr(jobs,jlev,jhor) = &
...
!
    ...
    REAL,    INTENT(OUT) :: slant   ! path length (m)
		       
!
! local variables
!

    INTEGER :: i,j,n,m,ibot,iray,k,kp1,idum,ilev,ihor
...
!
       ...
       y(3) = zenith_angle - alpha  ! thi - bending
...
! first calculation of derivs
             ...
             CALL pderivs(nlev,nhoriz,i,dsep,theta_min,theta_max,theta_tan,&
                  rtan,amult,refrac,radius,yt,dydht(:,1)) 
...
! second call at new yt
             ...
             CALL pderivs(nlev,nhoriz,i,dsep,theta_min,theta_max,theta_tan,&
                  rtan,amult,refrac,radius,yt,dydht(:,2)) 
...
! third call at new yt
             ...
             CALL pderivs(nlev,nhoriz,i,dsep,theta_min,theta_max,theta_tan,&
                  rtan,amult,refrac,radius,yt,dydht(:,3)) 
...
! fourth last call
             ...
             CALL pderivs(nlev,nhoriz,i,dsep,theta_min,theta_max,theta_tan,&
                  rtan,amult,refrac,radius,yt,dydht(:,4)) 
...
!
! estimate the slant delay form a linear combination of the two slant values
!

    IF (ABS(theta_ray(2) - theta_ray(1)) > 1.0E-10) THEN

       wt = (theta_ray(2) - theta_gps) / (theta_ray(2) - theta_ray(1))
...
! we could put a more complicated solver in here but I think this is adequate. 


    slant = wt * slant_tmp(1) + (1.0-wt)*slant_tmp(2)
...
!!  dndr=0.0 !!!!
    ...
    dydh(3) = -SIN(y(3))*(1.0/(y(1)+rtan) + dndr)
...
!
  SUBROUTINE twodimrefractivity( &
    ...
    REAL, PARAMETER :: k1=77.607E-2, k2=70.4E-2, k3=3.739E3, eps=0.622
    INTEGER :: jobs, jlev, jhor
    DO jobs=1,nobs_tot
       DO jlev=1,nlev+1
          DO jhor=1,nhor_used(jobs)
